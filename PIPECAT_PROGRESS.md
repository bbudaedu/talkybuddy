...
👂 聽成：可以教我說英文嗎？   ← 完全正確
👂 聽成：有聽到我說話嗎？     ← 完全正確
完成 5 輪 │ 194 個音訊 chunk
```

**SenseVoice 對真人語音表現很好**，且是在寶貝多米背景音樂下。

同輪發現三個問題並全部修掉：

| 問題 | 根因 | 修法 |
|---|---|---|
| ctx 爆掉（516→579→642） | 照抄 pipecat 累積歷史，沒照抄現行的無狀態設計 | `StatelessContextProcessor` |
| 聽到自己（「跟我說一定方」）、自我打斷 3 次 | `AlwaysUserMuteStrategy` 只擋 LLM 輸入，不擋 VAD/STT | `PlaybackGate` 攔在 VAD 之前 |
| 看不到玩偶回覆 | `LLMTextFrame` 被 TTS 消費，探針放在 TTS 之後（**同一錯誤犯三次**） | 探針移到 TTS 之前 |

## ✅ 閘門驗證通過（第十五輪）

```
疑似自我打斷次數：0          ← 上一輪是 3
🗣 玩偶說：你說得好！跟我說一遍：I want an apple。
```

同時證實教材注入與帶讀護欄正確運作（帶讀的是英文目標句，不是中文）。
...
寶貝多米觸發 VAD 3 次。**與記憶 `project-edge-s2s-tuning` 記的「把噪音判成韓文
字符」是同類現象。**

⚠️ **我先前「Silero 對背景零誤觸」的結論下得太早**——那是 10 秒樣本，
90 秒實跑就出現了。

`live_client` 早有兩道防線，pipecat 版都還沒接：

1. **按鍵觸發**（power 鍵開關一段 session，待機時完全不送音訊）
2. **`is_near_field()`** 近場門檻，用峰值擋掉遠處電視與旁人
