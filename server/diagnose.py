...
def _build_companion_directive(flags: dict, scores: dict, prev: dict | None) -> dict:
    ...
    # fallback_hint 是給 LLM 讀的完整中文提示句；fallback_prompt 是同一個簡化策略
    ...
    # fallback_hint 的措辭刻意避開「退回／降階」這類字眼——這句話會被
    # format_directive_for_prompt() 原樣塞進 LLM 的 system prompt，LLM 很可能
    # 照著這個框架去生成語氣，寫成「退回」LLM 就容易講出「我們退步一點」
    # 這種讓學生覺得被貼標籤的話；改成「邀請玩遊戲/好奇」的框架。
    if flags["article_correction"]:
        goal = "冠詞 a/an 穩定使用（母音開頭名詞前用 an）"
        topic = "食物與水果"
        questions = ["Do you want an apple?", "Is it an egg or a banana?"]
        fb = "若學生答不出來，別讓他覺得退步，改用邀請玩選擇遊戲的語氣問：Is it a or an?"
        fallback_prompt = "Is it a or an?"
    elif flags["high_chinese_ratio"]:
        goal = "整句英文輸出、減少中文替代"
        topic = "今天的一天"
        questions = ["What did you eat today?", "How are you today?"]
        fb = "若學生一直說中文，用好奇鼓勵的語氣邀請他先試著說一個英文單字就好，不要讓他覺得是被簡化。"
        fallback_prompt = "apple"
    elif flags["short_sentences"]:
        goal = "把句子講長、加上形容詞或地點"
        topic = "顏色與數量"
        questions = ["What color is it?", "How many do you have?"]
        fb = "若學生只說單字，用好玩選擇題的語氣邀請：Is it big or small?"
        fallback_prompt = "Is it big or small?"
    elif difficulty == "up":
        goal = "升級句型：用 and / because 造複合句"
        topic = "喜歡的事物與原因"
        questions = ["What do you like and why?"]
        fb = "若學生卡住，別讓他覺得退步，改用邀請好奇的語氣問：What do you like?"
        ...
        fb = "若學生困惑，用好玩選擇題的語氣邀請：What is your name?"
