...
### ✅ 數值正確性驗證（2026-07-28 補做）

以 `benchmark_model` 的 `--input_layer_value_files` 餵入固定輸入、
`--output_filepath` 匯出輸出張量，三方比對。

**輸入取自真實語句**：把 `/Mul_7_output_0` 加為完整 Piper ONNX 的額外輸出，
以 espeak-ng 音素化一句中文後跑完整模型，擷取真正的 vocoder 輸入 z
（`[1,192,339]`，取前 200 frames）。

> **為什麼不能用亂數測**：先用 N(0,1) 高斯亂數測過一輪（SNR 55.2 dB），
> 但真實 z 的 `rms=2.16`、範圍 **±85**，而高斯亂數 `rms=1.0`、範圍約 ±5——
> **動態範圍差 17 倍**，FP16 的誤差行為可能完全不同。
> 實測後兩者 SNR 相近（55.2 vs 57.9 dB），亂數測試結論僥倖成立，
> 但**方法上必須用真實輸入**，此處記錄以免日後照抄錯誤做法。

| 比對 | 最大絕對誤差 | SNR | 相關係數 |
|---|---|---|---|
| ONNX vs TFLite-CPU | 0.000001 | **121.4 dB** | 1.000000 |
| ONNX vs TFLite-NPU | 0.001257 | **57.9 dB** | 1.000000 |
| TFLite-CPU vs TFLite-NPU | 0.001257 | 57.9 dB | 1.000000 |

**結論一：`mtk_converter` 的轉換實質無損。** 121.4 dB 已是 float32 捨入層級，
轉檔沒有引入任何有意義的誤差。

**結論二：NPU 的 57.9 dB 來自 MDLA 強制的 FP16 運算**
（`NEURON_FLAG_USE_FP16` 為強制，見 §8.4），約等於 9.6 bit 有效位元。
訊號峰值 0.4911（-6.2 dBFS），同音量下 16-bit 量化雜訊約 89.8 dB，
**因此 FP16 誤差高於 16-bit 量化雜訊**，不能宣稱「數學上聽不出來」。
參考點：電話 35–40 dB、FM 廣播 50–60 dB、CD 96 dB。

已產生對照音檔並在裝置上以 3.5mm 喇叭 A/B 播放，**待使用者聽測判定**。

> ⚠️ **音檔內容可能無意義**：espeak-ng 以 `-v cmn` 處理繁體中文時輸出的 IPA
> 混入英語音素，合成內容大概率是亂碼。**這不影響本次的雜訊比對目的**
> （兩段用完全相同的 z，差異只來自 FP16），但若要做語意層面的聽測需另備正確音素。

### 效益right-sizing（2026-07-28 實測後修正，**先前的估算過高**）

原評估依 Phase 8 的 `tts_first`=1209ms 推得「TTS 佔穩態回合 40.8%」，
並據此把 vocoder 當成高價值標的。**實測後這個框架需要修正。**

裝置端原生迴路實測（同一 process 三輪，見 §「原生音訊迴路」）：

| | 第 1 輪（冷） | 第 2 輪（暖） | 第 3 輪（暖） |
|---|---|---|---|
| TTS 總時間 | 2.66s | **0.81s** | 0.82s |
| 產生音訊長度 | 3.02s | 3.06s | 3.29s |
| ASR | 2.30s | **0.31s** | 0.31s |

**推算 vocoder 的實際佔比**：

- TTS 暖機後總計 **810ms** 產生約 3.0 秒音訊
- vocoder 在 CPU-6T 為 243.7ms／2.32 秒音訊 → 3.0 秒約 **315ms**
- → **vocoder 佔 TTS 約 39%**（315／810）

**因此 NPU 化 vocoder 的實際節省上限**：
315ms → 約 39ms（30.4ms × 3.0/2.32），**每句省約 276ms**。
對照 Phase 8 穩態回合 2,960ms，約為 **9.3%**。

> **這比先前的框架小得多。** 8.0× 的加速是真的，但它作用在只佔 TTS 39%、
> 佔整個回合不到 10% 的區段上。**振幅大不等於影響大**——
> 先前把「TTS 佔回合 41%」直接當成 vocoder 的可節省空間，是把整個 TTS
> （含 enc_p 865 節點 + dp 1,455 節點 + flow 196 節點）都算進去了，那些不會上 NPU。
>
> `推導` 標示：276ms 與 9.3% 皆為由上述實測值計算而得，非直接量測。
> 真正的驗證要等兩段式推論接線完成後量整條 TTS。

### 尚未完成、不得粉飾

- **未接進產品。** 目前只證明 vocoder 子圖可在 MDLA 上跑、延遲 8.0×、數值正確。
  要真正省到時間，必須把 piper 的單次 ONNX 推論改成兩段式
  （enc_p+dp+flow 走 CPU → vocoder 走 NPU），**這步尚未做，且是本方案最大風險**。
  現行 TTS 走 sherpa-onnx `OfflineTts`（`server/tts.py`），它把 VITS 整個包起來，
  **要拆成兩段必須繞過 sherpa-onnx 自行接 ONNX Runtime**，工程量不小。
- **真實語句的 T 是 339 frames（3.94 秒），而 TFLite 釘死在 T=200（2.32 秒）。**
  需要決定分段或 padding 策略，且每段固定成本 30.4ms、初始化 1.16 秒
  （初始化成本高，必須常駐不可每次重建）。
