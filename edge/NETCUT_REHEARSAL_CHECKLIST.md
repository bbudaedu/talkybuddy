...
> **開機後的前置動作已固化，見 `edge/BOOT_SOP.md`。** 一行自檢取代人工逐項確認：
> `./.venv/bin/python -m edge.runtime.preflight --mic`（任何 ❌ 都會直接寫出修法）。
> 唯一無法自動化的手動步驟是**按 USB 麥克風的實體靜音鍵**。

## ✅ 已備妥的前置（2026-07-29 實測確認）

| 項目 | 狀態 |
|---|---|
| SSH `root@192.168.31.78` | ✅ 可連（會斷，但原因是上游網路而非裝置，見下方風險 3） |
...
3. **遠端連線會斷——但原因是上游網路，不是裝置**（2026-07-29 使用者澄清，
   先前歸因錯誤）。網路來源是**使用者的手機網路分享給路由器**，不穩或被拔線
   時整條就斷。2026-07-29 一天內斷了 4 次。

   **怎麼分辨是網路斷還是看門狗重開機**：恢復後看 `uptime`——若是連續的
   （例：9:43 → 10:15）就純粹是網路，裝置從未重開、stack 行程 PID 不變。

   ```bash
   ssh root@192.168.31.78 'uptime; pgrep -af "uvicorn server.app|llama-server --model" | wc -l'
   ```

   **決賽現場仍應規劃為完全離線操作**——理由是現場網路環境不可控，
   不是裝置本身會失聯。若現場能接乙太網路（已驗證 UP/1Gbps），風險大幅下降。
