...
| **5** | **2026-07-29 稍晚** | **B（真實斷網）** | **0.836** | `blocked` | **GO（僅 M1）** | ✅ **首次成功執行** `probe_netcut_b.py`。回合開始後 411ms `pkill` 反向隧道；該回合 `llm=4083ms`，切斷後純 edge 回合 `llm=3247ms` → M1 = **836ms**。`threads=6`、`CLOUD_LLM_TIMEOUT_S=4`。⚠️ **失效型態是「連線被拒」不是「無回應」**，見下方限制 4 |
| 6 | 2026-07-29 稍晚 | 雲端基準 | — | — | — | 同一場：暖場雲端回合 `llm=2516ms`；另一回合 `llm=7619ms round=8540ms` = **雲端逾時 4s 後降級 edge 3.2s**。雲端回合會落在 2.5s 或 7.6s 兩群，取決於中轉當下是否 > 4s |
| 7 | 2026-07-29 稍晚 | 純 edge 基準 | — | — | — | `threads=6` 連續 4 輪：`llm` 3,045–4,011ms、`round_total` 3,861–5,166ms（同一支 `probe_latency.py`）|

**至少 3 列實測，且其中至少 1 列型態為 B**。未實測一律留空或記 `blocked`，**禁止填入未實際量到的數字**。

> **NETCUT-03 的「至少 1 列型態 B」要求，已由第 5 列滿足**（2026-07-29 稍晚）。
> M2（可聽見回覆的牆鐘時間）全部仍是 `blocked`——需要碼錶＋麥克風＋瀏覽器，
> 且**需要使用者在場**，無法由 API 路徑取得。

### ⚠️ 本次演練的限制（讀結果表前必看）

> **限制 1、2 已於 2026-07-29 稍晚解決**（第 5 列＋`threads=6`）。限制 3 仍在。
> **新增限制 4，讀第 5 列的 836ms 前必看。**

1. ~~**型態 B 尚未真正測到降級**~~ → **已解決**（結果表第 5 列）。原因是方法論問題：
...
3. **M2 全部 `blocked`**：需要碼錶/錄影記錄牆鐘時間，且要走真實的麥克風與
   瀏覽器路徑，本次 API 路徑量不到。
4. **⚠️ 第 5 列的 M1 = 836ms 是最好的情況，不是最壞的情況。**
   `probe_netcut_b.py` 用 `pkill` 殺掉反向隧道，裝置那端得到的是 **connection
   refused（連線被拒）——立刻失敗**。但**現場拔網路線的失效型態是「無回應」**：
   封包石沉大海，要等到 `CLOUD_LLM_TIMEOUT_S` 到期才降級。

   | 失效型態 | 現場對應 | M1 |
   |---|---|---|
   | 連線被拒（本次量到） | 上游服務掛掉／隧道斷 | **0.836s** |
   | 無回應（**未量到**） | **拔網路線、WiFi 掉線** | 上界 ≈ `CLOUD_LLM_TIMEOUT_S` = **4s** |

   **簡報與 GO/NO-GO 判定應採用 4s 這個上界**，因為那才是現場最可能發生的型態。
   4s 超過本文件 §2 原訂的 3.0s 門檻——**要嘛調低 `CLOUD_LLM_TIMEOUT_S`，要嘛
   在結果表註明採用值並說明理由**。調低的代價是雲端回合更容易被砍掉
   （中轉中位數 1,883ms、上緣 4,020ms，見結果表第 6 列的雙峰現象）。

   要量到「無回應」型態，`pkill` 要換成**丟棄封包而非拒絕**，例如在裝置上
   `iptables -A OUTPUT -p tcp --dport 8317 -j DROP`（測完務必 `-D` 移除）。
