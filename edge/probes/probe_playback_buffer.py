...
## ⚠️ 這支會給假綠燈，除非在真實負載下跑（2026-08-01 血淚）

第一次用它得到「500,000 µs + keepalive = 0 underrun」，據此把板子的緩衝從
2.0s 調到 0.5s。**真跑起來 underrun 從每小時 16 次暴增到 290 次，最大一次
9.1 秒**，已回退。

原因是**跑這支時服務是停的**（上面那行指令自己就寫著 `systemctl stop`）——
沒有 VAD／STT／LLM 在搶 CPU（實測 pipeline 光是閒置就吃 1.3 核）。keepalive 是
asyncio task，被延遲就餵不上；而它每次寫入的量恰好是 **1.0 倍實時速率、沒有
任何餘裕**，所以只要有排程抖動，小緩衝立刻餓死。

**要用這支下結論，必須讓真實 pipeline 同時在跑**，否則測到的是一個不存在的
情境。更可靠的做法是直接看服務跑起來之後的 journal：

    journalctl -u talkybuddy-pipecat -b | grep -ci underrun
