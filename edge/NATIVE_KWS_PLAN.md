...
### ~~階段 0：解除麥克風阻擋~~ → ✅ **已完成，並已端到端驗證（2026-07-28）**

USB 麥克風 `plughw:1,0` 收音正常、3.5mm 喇叭 `plughw:0,0` 播放經使用者聽測確認，
mixer 設定已 `alsactl store` 持久化。完整組態與兩個坑見 §5。

**進一步做了全原生迴路驗證**：裝置自行合成語音 → 喇叭播放 → 空氣 → USB 麥克風錄音
→ SenseVoice 辨識，**全程不經瀏覽器**。腳本 `loopback2.py`（同一 process 三輪）：

| | 第 1 輪（冷） | 第 2 輪（暖） | 第 3 輪（暖） |
|---|---|---|---|
| TTS | 2.66s | **0.81s** | 0.82s |
| 音訊長度 | 3.02s | 3.06s | 3.29s |
| ASR | 2.30s | **0.31s** | 0.31s |
| 辨識結果 | ✓ | ✓ | ✓ |

原文「小朋友你好，今天我們一起學英文」，**三輪皆逐字正確、信心值 1.0**。

**這證明什麼**：

1. **「無螢幕獨立裝置」在音訊層已經成立**——裝置能聽、能說、能辨識，不需要瀏覽器。
   目前只剩喚醒詞還在瀏覽器端，這正是本文件要解決的最後一塊。
2. **ASR 暖機後 310ms**，與 Phase 8 的 405ms 相符，且這次是走真實聲學路徑
   （喇叭→空氣→麥克風）而非檔案輸入，**條件更嚴苛仍達成**。
