...
### ⛔ 真人聲音測試：不通過（2026-07-28～29）

**閉環自檢（TTS 合成音）通過，但真人聲音失敗。** 這個落差本身是重要結果——
**用自家 TTS 驗證 KWS 會給出假的信心**，因為 Piper 合成音音高平穩、咬字規整，
與真人語音差異極大。

四次真人錄音、約 11 次「說說學伴」，最佳結果為 **1 次命中**。

#### 逐項排除過程

| 假設 | 處置 | 結果 |
|---|---|---|
| 門檻太嚴 | 掃 0.05／0.10／0.15／0.20／0.25／0.35 | **完全無差別**，非門檻問題 |
| `keywords_score` 太低 | 掃 1.0／1.5／2.0／3.0／4.0 | **提高反而更差**，1.0 已最佳 |
| 模型檢查點 | 試 `epoch-12-avg-2` 與 `epoch-99-avg-1` | **行為完全一致** |
| 聲調標錯（學 ué vs 雪 uě） | 兩種聲調並列 | `uě` 版**完全無效**，證明 `ué` 才對；ASR 寫成「雪」只是同音字轉寫 |
| 喚醒詞太長 | 試「說說學」「說說」「學伴」 | 3 字 0 命中；2 字「說說」僅 1 次 |
| 錄音削波 | 增益 147→90→55（-0.94→-18.11dB）、關閉 AGC | **仍削波**，麥克風類比前端飽和 |
| 模糊比對（使用者提議） | 把 ASR 實際聽到的 9 種錯誤轉寫全登記為喚醒詞 | 0→1 次，**有改善但不足**，且命中的是「說說」「所說學」等易誤觸詞 |

#### 根因判定

四次錄音的 ASR 轉寫高度一致：「說梭水」「說說學」「初說雪」「搜梭學」「所說學」
「說說學霸」「初訟學」「そそ」——**「伴」這個字幾乎從未被辨識到**。
KWS 要求所有 token 全中，`b àn` 進不來就不可能命中。

兩個疊加成因，**皆非參數可解**：

1. **麥克風類比前端飽和。** 把 ALSA 增益從 147 降到 55（-18dB）、關閉 AGC 後，
   大聲講話仍 `peak=1.000`。這是硬體限幅，只能靠拉遠距離與降低音量緩解。
   削波失真直接毀掉高頻齒音，正是 `sh`／`x` 這些聲母最需要的頻段。
2. **這顆 wenetspeech KWS 模型對「說說學伴」的匹配度本身有限。**
   該模型內建的示範喚醒詞（你好军哥／小爱同学／小米小米）都是常見的雙聲疊詞或
   高頻詞組，「學伴」不在其分布內。

#### 決定

**決賽（2026-08-01）維持瀏覽器端 Porcupine，原生 KWS 列入 v2。**
剩餘時間不足以換模型或自訓喚醒詞，而音訊硬體的限幅問題需要換麥克風才能根治。

**但階段 0 的成果不受影響且已到手**：原生音訊迴路（TTS→喇叭→麥克風→ASR）
端到端驗證通過、逐字正確，`edge/runtime/wake_listener.py` 亦可運作——
只是喚醒詞的真人辨識率不足以上場。

#### 給 v2 的具體建議

1. **換麥克風**是前提。需要不會在近距離飽和的 USB 麥克風，購買前應實測 `peak`。
2. 喚醒詞改用**雙聲疊詞**（如「學伴學伴」）較符合這類模型的分布。
3. 或改用 Porcupine 的自訓 `.ppn`（需付費帳號但可離線跑），繞開 KWS 模型的分布限制。
4. **不要再用 TTS 合成音驗證 KWS**——它會給出假的通過。
