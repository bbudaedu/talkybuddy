...
| pipecat + sherpa 行程 RSS | **664MB** | ⚠️ 板子可用僅 1.7G |
| 磁碟增量 | 約 600MB | 裝完剩 4.0G |
| edge↔server RTT / 頻寬 | 116ms / ~550kB/s | 且**會整條斷** |
...
1. **完整 pipeline 的 RSS**——單是 pipecat+sherpa 就 664MB，加上 TTS voice
   （63MB 模型）與 llama-server（本身吃掉 2.0G 的大部分），是否還塞得進 1.7G。
   **這是目前最大的未知數**，量測因板子斷線而未完成。
2. **端到端 round_total**——未跑過。預期仍是約 4–5 秒（LLM 主宰），
   若量出來明顯更差，代表 pipecat 的編排開銷不可忽略。
3. **CPU 爭用**——VAD 只吃 6% 即時率，但 VAD+STT+TTS 同時跑、且與 llama-server
   搶 8 核，未量測。
...
5. **TTS 不是串流的**——`TTSEngine.synth()` 一次合成整段才回傳，
   first-audio 延遲 = 整句合成時間。切 chunk 不會讓第一個 byte 早出來。
