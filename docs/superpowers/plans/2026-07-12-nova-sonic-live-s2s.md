...
## Task 8/9 前端錄放整合細化（2026-07-13 修訂）

> 取代上方 Task 8 Step 1/3 的純函式集與整個 Task 9。定位改為 **live 為主 + 自動降級**（見 spec 2026-07-13 修訂章節）。後端 Task 1–7 已完成不動。inline 執行、TDD、勿委派 subagent。

### Task 8'：純函式集（含新增 `downsampleTo16k`）+ node 測

**Files:** Create `web/live-client.js`；Test `web/live-client.test.mjs`（已存在未追蹤的 3 test，補第 4 個）

- [ ] **Step 1**：在 `web/live-client.test.mjs` 追加 downsample 測：

```javascript
import { downsampleTo16k } from "./live-client.js";
...
- [ ] **Step 2**：`node --test web/live-client.test.mjs` → FAIL（找不到 `./live-client.js`）
- [ ] **Step 3**：建 `web/live-client.js` 純函式段（4 export）：
...
- [ ] **Step 4**：`node --test web/live-client.test.mjs` → PASS（4 tests）
- [ ] **Step 5**：commit `feat(live): live-client 純函式（PCM16/base64/播放佇列/降採樣）+ node 測`

### Task 9'：整合層 `LiveSession` + capture worklet + index.html live 為主接線

整合層無自動斷言，正確性靠真機 e2e。**Files:** Create `web/live-capture-processor.js`；Modify `web/live-client.js`（append 整合層）；Modify `web/index.html`；Create `docs/superpowers/plans/2026-07-12-nova-sonic-live-s2s-e2e-checklist.md`

- [ ] **Step 1**：建 `web/live-capture-processor.js`（AudioWorklet processor）：
...
- [ ] **Step 2**：`web/live-client.js` append 整合層：`liveWsURL()` + `class LiveSession`。
  - `constructor({onTranscript,onTurnEnd,onError,onStateChange})`
  - `async start()`：開 `/ws/live` WS（binaryType arraybuffer）→ 開播放 `AudioContext({sampleRate:24000})` → `getUserMedia({audio:{channelCount:1}})` → 擷取 `AudioContext` + `audioWorklet.addModule("/static/live-capture-processor.js")` → `AudioWorkletNode("live-capture")`；`node.port.onmessage`：`capturing` 時 `floatTo16BitPCM(downsampleTo16k(frame, ctx.sampleRate))` → `ws.send`（binary）；node 經 0-gain 靜音接 destination（避免回授）。
  - `pushToTalkStart()`：`capturing=true` + `onStateChange("listen")`
  - `pushToTalkEnd()`：`capturing=false` + `ws.send({type:"user_end"})` + `onStateChange("talk")`
  - WS `onmessage`：binary → `_play(Int16Array)`；text → 分派 `live_transcript`/`turn_end`/`live_error`
  - `_play(int16)`：int16→float32→`playCtx.createBuffer(1,len,24000)`→`BufferSourceNode`，`nextStart=max(nextStart,currentTime)`；`start(nextStart)`；`nextStart+=duration`（排程接續）
  - `async close()`：送 `{type:"bye"}`、關 ws、停 mic tracks、`ctx.close()`、`playCtx.close()`（各自吞例外）
- [ ] **Step 3**：`web/index.html` 定位翻轉接線：
  - `init()` 讀 `/api/status` 後：`var forceHalf = sessionStorage.getItem("forceHalfDuplex"); if (forceHalf) sessionStorage.removeItem("forceHalfDuplex"); if (st.live_s2s && !forceHalf) enterLiveMode();`
  - `async function enterLiveMode()`：先 `await window._wakeController.engine.stop()`（若有、吞例外）停喚醒常駐麥、**不建 `/ws/talk`**；`import("/static/live-client.js")` → `new LiveSession({onTranscript:顯示泡泡, onTurnEnd:setVisualState("idle"), onError:degradeToHalfDuplex, onStateChange:setVisualState})`；`await session.start()`（catch→`degradeToHalfDuplex("stream_error")`）；`window.__liveSession=session`；把 hold-to-talk 綁到主互動鈕（`pointerdown→pushToTalkStart`、`pointerup/pointercancel/pointerleave→pushToTalkEnd`）。
  - `function degradeToHalfDuplex(reason){ showToast(<reason 對應訊息>); sessionStorage.setItem("forceHalfDuplex","1"); setTimeout(function(){location.reload();},1500); }`
  - reason 訊息：`consent_required`→需同意錄音上雲；`unavailable`→即時對話暫不可用；`stream_error`→連線中斷改用一般模式。
- [ ] **Step 4**：建 e2e checklist（沿用原 Task 9 Step 4 內容，補「進頁自動進 live」「hold-to-talk」「live_error 自動降級重整回半雙工」三項）。
- [ ] **Step 5**：`node --test web/live-client.test.mjs`（4 綠）+ `.venv/bin/python -m pytest tests/ -q`（後端回歸）
- [ ] **Step 6**：commit `feat(live): 前端錄放整合（LiveSession/AudioWorklet/hold-to-talk、live 為主+自動降級）`

### 真機 e2e（Task 10'，手動）
用戶提供短期 SigV4 → 本機 HTTPS 起 server → `/api/status` live_s2s=true → 瀏覽器 hold-to-talk 說中文 → 見逐字稿 + **聽到**中文語音 + `/api/interactions` 有 `source=live_s2s` → 驗完撤憑證。
