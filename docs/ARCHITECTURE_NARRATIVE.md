...
**2026-07-29 更新：M1 已實測。** `edge/NETWORK_CUT_REHEARSAL.md` §5 第 5 列——
型態 B（回合進行中真的切斷雲端路徑）**M1 = 0.836 s**。

> ⚠️ **但簡報要講的是「1 秒內決定降級，最壞 4 秒」，不是「0.8 秒」。**
> 那次量到的失效型態是**連線被拒**（隧道被殺，立刻失敗）；而現場拔網路線是
> **無回應**——封包石沉大海，要等 `CLOUD_LLM_TIMEOUT_S`（現為 4 s）到期。
> 兩者差 5 倍，講小的那個會在現場被打臉。

`未證`：M2（孩子實際聽見回覆的牆鐘時間）仍全部 `blocked`，需要碼錶＋麥克風＋
瀏覽器，API 路徑量不到。**簡報不得給 M2 任何數字。**
...
| 常數 | 程式碼預設 | **裝置實際值** | 理由 |
|---|---|---|---|
| `cloud_llm._TIMEOUT_S` | 1.5 s | **4 s** ⚠️ | 雲端沒回就快點放棄，讓降級快 |
| `config.CLOUD_TTS_TIMEOUT_S` | **1.5 s** | 1.5 s | 同上 |
| `pipeline.LLM_TIMEOUT_S` | **8.0 s**（維持寬鬆） | 8.0 s | 真機 edge 引擎最壞 4,170 ms，砍短會**把本地餓死** |

> 一句話講給評審聽：**「我們縮短的是『放棄雲端』的時間，不是『等待本地』的時間。」**

> ⚠️ **2026-07-29 更正：裝置上跑的是 4 s，不是 1.5 s**（`.env` 的
> `CLOUD_LLM_TIMEOUT_S=4`）。若被問「多久放棄雲端」，**誠實答案是 4 秒**。
>
> **為什麼調成 4**：經反向中轉的雲端延遲中位數 **1,883 ms**（上緣 4,020 ms），
> 用 1.5 s 會讓雲端**每次都被砍掉**——「雲端模式」實際上永遠跑 edge，
> 那比降級慢更糟，因為它讓整個雙模式論述變成假的。
>
> **這是個真實的取捨，不是疏漏**：
>
> | 逾時值 | 雲端成功率 | 降級延遲 |
> |---|---|---|
> | 1.5 s | 幾乎全部被砍（中位數就超過了） | 快 |
> | **4 s（現行）** | 2026-07-29 實測 3/4 成功 | 最壞 4 s |
>
> **決賽當天若裝置能直連網際網路**（不走反向中轉），少一個 hop，延遲分布會變好，
> 屆時可考慮調回 1.5 s——**但那要現場實測後才能改，不能假設**。
...
|---|---|---|---|---|---|
| 1 | **即時陪聊**（孩子聽到的那句） | edge 優先 | **4.0–5.1 s** `2026-07-29 實測` ⚠️ | 降到規則式鷹架 | 課綱題庫 + 鷹架句型 |
...
> ⚠️ **第 1 列的延遲數字曾寫「穩態 2.96–2.99 s」，2026-07-29 更正。**
> 那三筆是複測同一句話量到的（KV cache 命中 292/293），真實對話每句都不同、
> 共同前綴只有 8 個字，拿不到那個命中率。證據與對帳見
> `edge/LATENCY_2.96S_EXPLAINED.md`。
> **對外一律講 4–5 秒，而且重點放在「斷網時完全不變」——賣點是不中斷，不是快。**
...
5. **隱私**（30 秒）：五道閘門；重點講 L3 為什麼是白名單，以及
   「有呼叫去識別化 ≠ 沒外洩」這個發現。
6. **收尾**（20 秒）：**1,115 個自動化測試** `實測` 守著這套行為，含一條真的把 WebSocket
   開著切換模式的端到端測試。
