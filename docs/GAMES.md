...
**裝置上沒有螢幕，現場實際是用講的開局**（不必打 curl）：

| 講這句 | 開哪一局 |
|---|---|
| 「我要玩小遊戲」 | 清單第一個（火眼金睛），並用講的報出其他三個名字 |
| 「我要玩背單字」 | 背單字 |
| 「不玩了」 | 結束這一局 |

意圖詞（我要／我想／玩／來一局）與遊戲名**兩個都要命中**才開局——
「背單字好難」是在聊天，不會擅自開局。

## 背單字上台前要先量一次（`spell_along` 專屬）

判定門檻是拿 TTS 合成音量出來的，**不是童聲**。上台前在裝置上跑：

```bash
ssh <裝置> 'cd /root/talkybuddy && .venv/bin/python -m edge.probes.probe_spell_child'
```

五個詞 × 三輪 ≈ 5 分鐘，跑完直接印一張表告訴你門檻該不該調：

- 現行門檻（0.6）過關率 **≥ 8 成** → 不用動，直接上
- 落在 **5–8 成** → 把 `server/spelling.py` 的 `PASS_THRESHOLD` 降到 0.5 再跑一次
- **低於 5 成** → 不是門檻的問題，先查麥克風收音（`edge/probes/probe_mic_gain.py`）

樣本不足 8 個時它會拒絕給建議——**只做三次也算得出中位數，而那個數字會
長得跟做滿一樣權威**。
...
- **孩子真聲的字母辨識率尚未驗證。** 實測用的是 TTS 合成音當代理，不是童聲。
  ...
  合成音的基準線：8 個詞各跑 5 輪共 40 個樣本，平均字母命中率 **0.83**，
  現行門檻 0.6 之下 38/40 過關。真機用童聲跑
  `edge/probes/probe_spell_recognition.py` 重量一次再決定要不要調門檻
- **本地 TTS 是隨機的**（VITS 的 duration predictor 帶噪聲）：同一個詞每次
  合成的音都不同，ASR 也就每次聽成不一樣的東西。**拼字相關的 spike 跑一次
  沒有意義**，要跑多輪看分布——第一次量測就因為只跑一輪而差點得出錯誤結論
