...
加了 PTT 之後真人測試發現**跟讀完全沒反應**——而跟讀是這個產品的核心互動。
兩件事同時擋住：孩子講完 PTT 就 disarm（要再按一次鍵），而且閘門還要 6.8 秒
才開。但跟讀的自然反應是立刻跟著唸，不是先按鈕。

**使用者裁定：玩偶講完自動開始聽。** 作法是把 `PlaybackGateFilter` 的「上行
重開」接出來（`on_reopen`），PTT 收到就 `arm()` 並嗶一聲。閘門重開的時機本來
就精準對應「玩偶講完且殘響過了」，不必另外算。

**噪音免疫力沒有損失**：15 秒沒講話仍回到「必須按鍵」，長時間待機照樣完全不聽。
等於 **PTT 守待機、VAD 守對話中的輪替**。

板子實測（2026-08-01 22:51，全程只按一次鍵）：
...
🎤 偵測到你開始說話…              ← 沒按鍵
👂 聽成：I see a dog.             ← 英文辨識正確
🗣 你唸得好清楚…跟我說一遍：I see a cat.     ← LessonProgress 換句
👂 聽成：I see. a cat.
🗣 …跟我說一遍：I see a rabbit.
```

四輪對話、三次跟讀、教材推進 dog→cat→rabbit→elephant→bird，全部自動。

### ✅ 緩衝 0.5s：這次連「講話期間」都驗過了

前面那段「500k 實跑爆掉」的結論**下得太早**——我只看 underrun 的**次數**就回退，
沒有看分布。分開統計之後：

| 期間 | underrun |
|---|---|
| 500k 閒置 3 小時 | 861 次，中位 1.28ms，**最大 18.7ms**，超過 100ms 的 0 筆，總掉音 0.019% |
| 500k **玩偶講話期間** | 最大 **3.3ms** |
| 唯一一筆 1652ms | 發生在 **STT 推論期間**（玩偶沒在播放，內容是 keepalive 靜音）→ 聽不到 |

對照歷史那批災難性的：**每次 0.8–1.9 秒**。完全不是同一個量級。

**現行設定：`TALKYBUDDY_EDGE_PLAYBACK_BUFFER_US=500000`**，死區 2.6s → 1.1s，
嗶聲從晚 2 秒變成晚 0.5 秒（使用者實測確認「B 聲有變快了」）。

⚠️ 大的 starvation 都落在 CPU 吃緊的 STT 那幾秒。目前它們剛好都在靜音期，
但那是**運氣好**不是設計保證——若哪天在播放期間撞上，就會是聽得到的斷音。
真要根治得讓 keepalive 有餘裕（寫得比實時稍快，靠緩衝滿了阻塞來調節），
**未實作未驗證**。
